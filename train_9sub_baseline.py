"""Baseline: all 9 subjects, fixed seed=42, no channel attention."""
import os
...
FIXED_SEED = 42
result_dir = 'compare_9sub_baseline'
...
for sub in range(1, 10):
    ...
    print(f"\n{'='*60}")
    print(f"  BASELINE - Subject {sub}, Seed {FIXED_SEED}")
    print(f"{'='*60}")
    ...
    print(f"\nBASELINE S{sub}: acc={acc*100:.2f}%, kappa={kappa*100:.2f}%, epoch={best_epoch}, time={elapsed:.1f}s")

    del exp
    torch.cuda.empty_cache()

print(f"\n{'='*60}")
print(f"  BASELINE SUMMARY (All 9 Subjects, Seed {FIXED_SEED})")
print(f"{'='*60}")
